...
const DEFAULT_CHECKPOINT_LIMIT = process.env.INKOS_ANDROID === "1" ? 24 : 100;
const DEFAULT_VARIANT_LIMIT = process.env.INKOS_ANDROID === "1" ? 6 : 20;
...
export class PlayStore {
  ...
  async saveCheckpoint(
    ...
    await this.ensureRun(worldId, runId);
    const checkpointDir = this.safeRunChildPath(worldId, runId, "checkpoints");
    await writeFile(
      this.safeRunChildPath(worldId, runId, join("checkpoints", `${assertSafeSegment(snapshot.id)}.json`)),
      `${JSON.stringify(snapshot, null, 2)}\n`,
      "utf-8",
    );
    await this.pruneSnapshotDirectory(checkpointDir, positiveEnvInt("INKOS_PLAY_MAX_CHECKPOINTS", DEFAULT_CHECKPOINT_LIMIT));
  }
  ...
  async saveVariant(
    ...
    const variantDir = join(this.runDir(worldId, runId), "variants", `turn-${turn}`);
    ...
    await this.pruneSnapshotDirectory(variantDir, positiveEnvInt("INKOS_PLAY_MAX_VARIANTS_PER_TURN", DEFAULT_VARIANT_LIMIT));
    return variantId;
  }
  ...
  private async pruneSnapshotDirectory(dir: string, limit: number): Promise<void> {
    const names = await readdir(dir).catch(() => [] as string[]);
    const files = await Promise.all(names
      .filter((name) => name.endsWith(".json"))
      .map(async (name) => ({ name, mtimeMs: (await stat(join(dir, name)).catch(() => null))?.mtimeMs ?? 0 })));
    files.sort((a, b) => b.mtimeMs - a.mtimeMs || b.name.localeCompare(a.name));
    await Promise.all(files.slice(limit).map(({ name }) => rm(join(dir, name), { force: true })));
  }
