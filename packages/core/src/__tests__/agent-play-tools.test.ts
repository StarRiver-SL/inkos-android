...
describe("agent play tools", () => {
  ...
  it("advances the play world bound to the session", async () => {
    ...
    const step = vi.fn(async () => STEP_RESULT);
    const runnerFactory = vi.fn(() => ({ step }));
    const tool = createPlayStepTool(pipelineStub(), root, sessionId, {
      playMode: "guided",
      runnerFactory,
    });
    ...
    expect(step).toHaveBeenCalledWith("我翻开账本看最后一页", { mode: "guided" });
    expect((await store.loadWorld(sessionId))?.mode).toBe("guided");
    expect(result.details).toMatchObject({
